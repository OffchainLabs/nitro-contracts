...
contract ChallengeEdgeLibAccess {
    ...
    function newEdgeChecks(
        ...
        return ChallengeEdgeLib.newEdgeChecks(
            originId, startHistoryRoot, startHeight, endHistoryRoot, endHeight
        );
    }
    ...
    function newChildEdge(
        ...
        return ChallengeEdgeLib.newChildEdge(
            originId, startHistoryRoot, startHeight, endHistoryRoot, endHeight, level
        );
    }
...
contract ChallengeEdgeLibTest is Test {
    ...
    function testNewLayerZeroEdge() public {
        ...
        assertEq(
            ...
            keccak256(
                abi.encodePacked(
                    e.level, e.originId, e.startHeight, e.startHistoryRoot, e.endHeight
                )
            ),
            "Id mem"
        );
    ...
    function testNewLayerZeroEdgeZeroClaimId() public {
        ...
        access.newLayerZeroEdge(
            originId, startRoot, 10, endRoot, 15, 0, staker, NUM_BIGSTEP_LEVEL + 1
        );
    }
    ...
    function testNewChildEdge() public {
        ...
        ChallengeEdge memory e =
            access.newChildEdge(originId, startRoot, 10, endRoot, 15, NUM_BIGSTEP_LEVEL + 1);
        assertEq(e.originId, originId, "Origin id");
        assertEq(e.startHeight, 10, "Start height");
        assertEq(e.startHistoryRoot, startRoot, "Start root");
        assertEq(e.endHeight, 15, "end height");
    ...
    function testEdgeExists() public {
        ...
        ChallengeEdge memory childEdge =
            child.newChildEdge(originId, startRoot, 10, endRoot, 17, NUM_BIGSTEP_LEVEL + 1);
    ...
    function testLength() public {
        ...
        ChallengeEdge memory childEdge = ChallengeEdgeLib.newChildEdge(
            originId, startRoot, 10, endRoot, 17, NUM_BIGSTEP_LEVEL + 1
        );
        child.setChallengeEdge(childEdge);
    ...
    function testSetChildrenTwice() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ChildrenAlreadySet.selector, child.id(), lowerChildId, upperChildId
            )
        );
        child.setChildren(lowerChildId, upperChildId);
    }
