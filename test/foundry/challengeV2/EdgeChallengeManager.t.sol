...
import "./Utils.sol";
import "../mocks/MockAssertionChain.sol";
...
contract EdgeChallengeManagerTest is Test {
        ...
        EdgeChallengeManager challengeManager = EdgeChallengeManager(
            address(
                new TransparentUpgradeableProxy(
                    address(challengeManagerTemplate), address(new ProxyAdmin()), ""
                )
            )
        );
    ...
    function testDeployInit() public {
        ...
        EdgeChallengeManager ecm = EdgeChallengeManager(
            address(new TransparentUpgradeableProxy(address(emt), address(new ProxyAdmin()), ""))
        );
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                StakeAmountsMismatch.selector, NUM_BIGSTEP_LEVEL, NUM_BIGSTEP_LEVEL + 2
            )
        );
        ecm.initialize(
            assertionChain,
        ...
        returns (EdgeInitData memory, bytes32[] memory, bytes32[] memory, bytes32)
    {
        ...
        assertEq(
            ei.challengeManager.firstRival(edge.mutualIdMem()),
    ...
    function testCanConfirmByTime() public {
        ...
        assertTrue(
            ei.challengeManager.getEdge(edgeId).status == EdgeStatus.Confirmed, "Edge confirmed"
        );
    }
    ...
    function testCanConfirmByTimeNotLayerZero() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                EdgeNotLayerZero.selector, blockEdges1[0].lowerChildId, ce.staker, ce.claimId
            )
        );
    ...
    function testCanConfirmByChildren() public returns (EdgeInitData memory, bytes32) {
        (EdgeInitData memory ei, bytes32[] memory states1,, bytes32 edge1Id) =
            testCanCreateEdgeWithStake();
        ...
        BisectionChildren memory children =
            bisect(ei.challengeManager, edge1Id, states1, 16, states1.length - 1);
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                CachedTimeSufficient.selector, challengePeriodBlock, challengePeriodBlock
            )
        );
        ei.challengeManager.updateTimerCacheByChildren(children.lowerChildId, challengePeriodBlock);
        ...
        assertTrue(
            ei.challengeManager.getEdge(edge1Id).status == EdgeStatus.Confirmed, "Edge confirmed"
        );
    ...
    function testRevertConfirmAnotherRival() public {
        ...
        BisectionChildren memory children =
            bisect(ei.challengeManager, edge2Id, states2, 16, states2.length - 1);
        BisectionChildren memory children2 =
            bisect(ei.challengeManager, children.lowerChildId, states2, 8, 16);
        _safeVmRoll(block.number + challengePeriodBlock);
        ...
        assertFalse(
            ei.challengeManager.getEdge(edge1Id).status
                == ei.challengeManager.getEdge(edge2Id).status
        );
    ...
    function testCantConfirmEdgeByTimeTwice() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(EdgeNotPending.selector, edge1Id, EdgeStatus.Confirmed)
        );
        ei.challengeManager.confirmEdgeByTime(edge1Id, ei.a1Data);
    }
    ...
    function bisectToForkOnly(
        ...
        winningEdges[4] = bisect(
            ...
            winningEdges[5].lowerChildId,
            args.winningLeaves,
            16,
            args.winningLeaves.length - 1
        );
        losingEdges[4] = bisect(
            args.challengeManager,
            losingEdges[5].lowerChildId,
            args.losingLeaves,
            16,
            args.losingLeaves.length - 1
        );

        // height 8
        winningEdges[3] =
            bisect(args.challengeManager, winningEdges[4].lowerChildId, args.winningLeaves, 8, 16);
        losingEdges[3] =
            bisect(args.challengeManager, losingEdges[4].lowerChildId, args.losingLeaves, 8, 16);

        // height 4
        winningEdges[2] =
            bisect(args.challengeManager, winningEdges[3].lowerChildId, args.winningLeaves, 4, 8);
        losingEdges[2] =
            bisect(args.challengeManager, losingEdges[3].lowerChildId, args.losingLeaves, 4, 8);

        winningEdges[1] =
            bisect(args.challengeManager, winningEdges[2].lowerChildId, args.winningLeaves, 2, 4);
        losingEdges[1] =
            bisect(args.challengeManager, losingEdges[2].lowerChildId, args.losingLeaves, 2, 4);

        // height 2
        winningEdges[0] =
            bisect(args.challengeManager, winningEdges[1].lowerChildId, args.winningLeaves, 1, 2);
        if (!args.skipLast) {
            losingEdges[0] =
                bisect(args.challengeManager, losingEdges[1].lowerChildId, args.losingLeaves, 1, 2);
        }
    ...
    function generateEdgeProof(
        ...
        bytes32[] memory claimEndInclusionProof = ProofUtils.generateInclusionProof(
            ProofUtils.rehashed(ArrayUtilsLib.slice(states1, 0, 2)), 1
        );
        bytes32[] memory edgeInclusionProof = ProofUtils.generateInclusionProof(
            ProofUtils.rehashed(bigStepStates), bigStepStates.length - 1
        );
        return abi.encode(
            states1[0],
            states1[1],
    ...
    function testRevertBigStepInvalidClaimType() public {
        ...
        bytes32 edge1SmallStepId;
        {
            ...
            (smallStepStates1, smallStepExp1) =
    ...
    function testCanConfirmByClaim() public {
        ...
        assertTrue(
            ei.challengeManager.getEdge(edges1[5].lowerChildId).status == EdgeStatus.Confirmed,
            "Edge confirmed"
        );
    }
        ...
        )
    {
        ...
        (BisectionChildren[6] memory edges1, BisectionChildren[6] memory edges2) = bisectToForkOnly(
            BisectToForkOnlyArgs(
                args.challengeManager, edge1Id, edge2Id, states1, states2, args.skipLast
            )
        );
    ...
    function testCanConfirmByClaimSubChallenge() public {
        ...
        assertTrue(
            ei.challengeManager.getEdge(allWinners[17].lowerChildId).status == EdgeStatus.Confirmed,
            "Edge confirmed"
        );
    }
        ...
        returns (EdgeInitData memory, BisectionChildren[] memory)
    {
        ...
        assertTrue(
            ei.challengeManager.getEdge(topEdgeId).status == EdgeStatus.Confirmed, "Edge confirmed"
        );
    ...
    function _updateTimerCacheByChildren(
        ...
        if (totalTimeUnrivaledCache >= maximumCachedTime) {
            vm.expectRevert(
                abi.encodeWithSelector(
                    CachedTimeSufficient.selector, totalTimeUnrivaledCache, maximumCachedTime
                )
            );
        } else {
    ...
    function _updateTimers(
        ...
        for (uint256 i = 1; i < allWinners.length; i++) {
            if ((i + 1) % 6 != 0) {
                if (i % 6 != 0) {
                    _updateTimerCacheByChildren(
                        ei.challengeManager, allWinners[i].lowerChildId, challengePeriodBlock
                    );
                } else {
                ...
                _updateTimerCacheByChildren(
                    ei.challengeManager, allWinners[i].upperChildId, challengePeriodBlock
                );
            } else {
                _updateTimerCacheByChildren(
                    ei.challengeManager, allWinners[i].lowerChildId, challengePeriodBlock
    ...
    function testCanRefundStake() external {
        ...
        // block level
        assertEq(
            afterBalance - beforeBalance, ei.challengeManager.stakeAmounts(level), "Stake refunded"
        );
    }
    ...
    function testRevertRefundStakeTwice() external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                EdgeAlreadyRefunded.selector, allWinners[allWinners.length - 1].lowerChildId
            )
        );
        ei.challengeManager.refundStake(allWinners[allWinners.length - 1].lowerChildId);
    }
    ...
    function testRevertRefundStakeNotLayerZero() external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                EdgeNotConfirmed.selector,
                allWinners[allWinners.length - 2].lowerChildId,
                EdgeStatus.Pending
            )
        );
    ...
    function testRevertRefundStakeNotConfirmed() external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(EdgeNotConfirmed.selector, edgeId, EdgeStatus.Pending)
        );
        ei.challengeManager.refundStake(edgeId);
    }
