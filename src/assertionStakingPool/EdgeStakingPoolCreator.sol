...
contract EdgeStakingPoolCreator is IEdgeStakingPoolCreator {
    ...
    function createPool(address challengeManager, bytes32 edgeId) external returns (IEdgeStakingPool) {
    ...
    /// @inheritdoc IEdgeStakingPoolCreator
    function getPool(address challengeManager, bytes32 edgeId) public view returns (IEdgeStakingPool) {
        return IEdgeStakingPool(
            StakingPoolCreatorUtils.getPool(type(EdgeStakingPool).creationCode, abi.encode(challengeManager, edgeId))
        );
    }
}
