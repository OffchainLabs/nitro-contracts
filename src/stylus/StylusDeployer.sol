...
contract StylusDeployer {
  ...
  function deploy(
    ...
    if (shouldActivate) {
      ...
      (, dataFee) = ARB_WASM.activateProgram{ value: activationValue }(
        newContractAddress
      );
    }
    ...
    if (initData.length != 0) {
      (bool success, ) = address(newContractAddress).call{ value: initValue }(
        initData
      );
      if (!success) {
