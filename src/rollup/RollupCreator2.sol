...
contract RollupCreator is Ownable {
    ...
    //  * @param args       The configuration for the rollup
     */
    function createRollup(RollupCreatorArgs memory args) public payable returns (address) {
        {
            // Make sure the immutable maxDataSize is as expected
            (, ISequencerInbox ethSequencerInbox, IInboxBase ethInbox, , ) = bridgeCreator
                .ethBasedTemplates();
            require(
                args._maxDataSize == ethSequencerInbox.maxDataSize(),
                "SI_MAX_DATA_SIZE_MISMATCH"
            );
            require(args._maxDataSize == ethInbox.maxDataSize(), "I_MAX_DATA_SIZE_MISMATCH");

            (, ISequencerInbox erc20SequencerInbox, IInboxBase erc20Inbox, , ) = bridgeCreator
                .erc20BasedTemplates();
            require(
                args._maxDataSize == erc20SequencerInbox.maxDataSize(),
                "SI_MAX_DATA_SIZE_MISMATCH"
            );
            require(args._maxDataSize == erc20Inbox.maxDataSize(), "I_MAX_DATA_SIZE_MISMATCH");
        }
        ...
        RollupProxy rollup = new RollupProxy{salt: keccak256(abi.encode(args))}();

        BridgeCreator.BridgeContracts memory bridgeContracts = bridgeCreator.createBridge(
            address(proxyAdmin),
            address(rollup),
            args._nativeToken,
            args.config.sequencerInboxMaxTimeVariation,
            args.config.dataHashReader
        );
        ...
        address upgradeExecutor = _deployUpgradeExecutor(args.config.owner, proxyAdmin);
        ...
        args.config.owner = address(this);
        ...
        if (args._batchPoster != address(0)) {
            bridgeContracts.sequencerInbox.setIsBatchPoster(args._batchPoster, true);
        }

        // Call setValidator on the newly created rollup contract just if validator set is not empty
        if (args._validators.length != 0) {
            bool[] memory _vals = new bool[](args._validators.length);
            for (uint256 i = 0; i < args._validators.length; i++) {
                _vals[i] = true;
            }
            IRollupAdmin(address(rollup)).setValidator(args._validators, _vals);
        }

        IRollupAdmin(address(rollup)).setOwner(address(upgradeExecutor));

        if (args._deployFactoriesToL2) {
            _deployFactories(
                address(bridgeContracts.inbox),
                args._nativeToken,
                args._maxFeePerGasForRetryables
            );
        }
